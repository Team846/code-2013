	...
	, m_userInTeleop(false)
{
	...
	printf("DriverStation::DriverStation() initialization started.\n");

	// Create a new semaphore
	m_packetDataAvailableSem = semBCreate (SEM_Q_PRIORITY, SEM_FULL);

	// Register that semaphore with the network communications task.
	// It will signal when new packet data is available. 
	//setNewDataSem(m_packetDataAvailableSem);
	DriverStationInput::StartLoop(m_packetDataAvailableSem);
	...
	printf("DriverStation::DriverStation() Driverstation initialized.\n");
}

DriverStation::~DriverStation()
{
	DriverStationInput::Dispose();
	m_task.Stop();
...
{
	printf("DriverStation::InitTask() Driverstation running!\n");
...
void DriverStation::Run()
{
	...
	while (true)
	{
		...
		semGive(m_packetDataAvailableSem);
		//if (m_userInDisabled)
		...
		// Sleep to allow the DriverStationInput thread to grab the semaphore.
		Sleep(2);
	}
}
